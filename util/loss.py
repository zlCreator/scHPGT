...
def loss_down(embedding, identity_matrix, size):
    loss = torch.mean(torch.abs(torch.triu(judge_emb(embedding), diagonal=1)))
...
class HVGEncodLoss(nn.Module):
    def __init__(self, dim=64, p =0.8, use_gpu = True):
        super(HVGEncodLoss, self).__init__()
        if use_gpu:
    ...
    def forward(self, atac_embeddings, rna_embeddings):
        ...
        rna_reduction_loss = loss_down(rna_embeddings[0], self.identity_matrix, self.dim)
        for i in range(1, len(rna_embeddings)):                
            rna_embedding_cat = torch.cat([rna_embedding_cat, rna_embeddings[i]], 0)            
            rna_reduction_loss += loss_down(rna_embeddings[i], self.identity_matrix, self.dim)    
                
        rna_reduction_loss /= len(rna_embeddings)
        
        # atac
        atac_reduction_loss = loss_down(atac_embeddings[0], self.identity_matrix, self.dim)
        for i in range(1, len(atac_embeddings)):
            atac_reduction_loss +=  loss_down(atac_embeddings[i], self.identity_matrix, self.dim)
...
class PrioritizedLoss(nn.Module):
    def __init__(self):
        super(PrioritizedLoss, self).__init__()
